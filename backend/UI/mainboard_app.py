...
from backend.utils.utils_2.xlsx_m2_1 import convertir_xls_a_xlsx_1
from backend.modules.Modules_2.procesar_motherboard_2 import procesar_numbers_desde_listas
...
def leer_filas_amarillas(path_excel):
    ...
    try:
        if engine == "openpyxl":
            ...
            required = ["MAINBOARD PART NUMBER", "PLANT", "INTERNAL MODEL"]
            for r in required:
                if r not in headers:
                    raise Exception(f"No se encontró la columna: {r}")

            col_mother   = headers["MAINBOARD PART NUMBER"]
            ...
            col_mother   = col_idx("MAINBOARD PART NUMBER")
...
def marcar_procesado(path_excel, mother_name):
    ...
    try:
        ...
        data  = sheet.used_range.value
        headers = [str(h).strip().upper() for h in data[0]]
        col_mother = headers.index("MAINBOARD PART NUMBER")
...
class MainboardApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("MBAutomator - Mainboards")
    ...
    def _procesar_archivo(self, excel_file: str):
        ...
        for idx, (mother, plant) in enumerate(zip(mothers, plants), start=1):
            ...
            excel_salida = os.path.join(MAINBOARD_2_FILES_FOLDER, f"{mother}.xlsx")
