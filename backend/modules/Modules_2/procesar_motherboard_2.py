...
from backend.utils.txt_to_xlsx import convertir_xls_a_xlsx
from backend.utils.utils_2.xlsx_m2_1 import exportar_bom_a_xls_1, convertir_xls_a_xlsx_1
...
def procesar_numbers_desde_listas(session, mother_list, plant_list, excel_output, capid=FILTRO, loop_multiple=False):
    ...
    for idx, mother in enumerate(mother_list):
        ...
        try:
            ...
            #! Exportar BOM
            ruta_xls = exportar_bom_a_xls_1(session, mother)
            ...
            ruta_xlsx = os.path.join(
                os.path.dirname(ruta_xls),
                f"{mother}.xlsx"
            )

            convertir_xls_a_xlsx_1(ruta_xls, ruta_xlsx)
