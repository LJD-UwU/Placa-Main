...
def procesar_archivo_principal_mainboard_2(
    ...
    # Convertir columnas manuales a float
    ...
    if lista_pcb:
        ...
        # Asignar LEVEL 2 a submateriales y manuales
        df_sub_normales["LEVEL"] = 2
        df_sub_final["LEVEL"] = 2
        df_manuales["LEVEL"] = 2

        # Buscar índices de LEVEL 2
        indices_lvl2 = df.index[df["LEVEL"] == 2].tolist()

        if len(indices_lvl2) >= 1:
            # Índice donde insertar (antes del último LEVEL 2)
            indice_insercion = indices_lvl2[-1]
        else:
            # Si no hay LEVEL 2, insertar al final
            indice_insercion = len(df)

        # Insertar submateriales normales y manuales antes del último
        df = pd.concat([
            df.iloc[:indice_insercion],
            df_sub_normales,
            df_manuales,
            df.iloc[indice_insercion:]
        ], ignore_index=True)

        # Agregar los finales hasta el mero final
        df = pd.concat([df, df_sub_final], ignore_index=True)

    # REINICIO ITEM / LEVEL DESPUÉS DE INSERTAR FILAS
    df["ITEM"] = df["ITEM"].apply(limpiar_item).astype(str)
    df.loc[~df.index.isin(filas_protegidas), "LEVEL"] = 0
    contador_bloque = 10
    for i, val in df["ITEM"].items():
